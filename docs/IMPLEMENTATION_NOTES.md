...
- Tooling: `scripts/validate_ppo_telemetry.py` checks NDJSON schema and reports per-epoch/aggregate drift (absolute + optional relative), `scripts/telemetry_watch.py` tails runs with alert thresholds (`kl`, `loss`, `grad`, `entropy`, `reward_adv_corr`) and can emit JSON for hooks, `scripts/telemetry_summary.py` generates text/markdown/JSON summaries, and `docs/notebooks/telemetry_quicklook.ipynb` offers quick-loss visualisation updated for telemetry_version 1.1 fields.
