...
- Stage 3C kicked off: `TrajectoryService` can flush transitions from DTO envelopes and regression coverage (`tests/policy/test_trajectory_service_dto.py`) locks the map/feature/anneal payloads expected by PPO/BC consumers.
