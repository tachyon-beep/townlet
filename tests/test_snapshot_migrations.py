...
def test_snapshot_migration_applied(tmp_path: Path, sample_config: SimulationConfig) -> None:
...
def test_snapshot_migration_multi_step(tmp_path: Path, sample_config: SimulationConfig) -> None:
    manager = SnapshotManager(tmp_path)
    initial_state = replace(_basic_state(sample_config), config_id="v1.0")
    snapshot_path = manager.save(initial_state)

    register_migration("v1.0", "v1.1", lambda state, config: replace(state, config_id="v1.1"))

    def migrate_to_target(state: SnapshotState, config: SimulationConfig) -> SnapshotState:
        return replace(state, config_id=config.config_id)

    register_migration("v1.1", sample_config.config_id, migrate_to_target)

    loaded = manager.load(snapshot_path, sample_config)
    assert loaded.config_id == sample_config.config_id
    assert len(loaded.migrations["applied"]) == 2


def test_snapshot_migration_missing_path_raises(tmp_path: Path, sample_config: SimulationConfig) -> None:
    manager = SnapshotManager(tmp_path)
