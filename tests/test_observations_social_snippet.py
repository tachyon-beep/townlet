...
def test_social_snippet_vector_length(base_config: SimulationConfig) -> None:
    service = WorldObservationService(config=base_config)
    ...
    assert features.shape[0] == len(service._feature_names)

    aggregate_names = getattr(service, "_social_aggregate_names", [])
    for name in aggregate_names:
        idx = service._feature_index[name]
...
def test_relationship_stage_required(base_config: SimulationConfig) -> None:
    ...
    with pytest.raises(ValueError, match="relationships stage"):
        WorldObservationService(config=config)
...
def test_disable_aggregates_via_config(base_config: SimulationConfig) -> None:
    ...
    assert not service._aggregate_names()
    ...
    agg_indices = [
        idx
        for name, idx in service._feature_index.items()
...
def test_observation_matches_golden_fixture(base_config: SimulationConfig) -> None:
    service = WorldObservationService(config=base_config)
    world = _build_world(base_config)
    obs = service.build_batch(world, terminated={})
    ...
    assert service._feature_names == names
