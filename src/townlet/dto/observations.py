...
class ObservationMetadata(_FrozenModel):
    """Metadata describing observation structure and variant."""

    variant: str  # "full", "hybrid", or "compact"
    map_shape: tuple[int, int, int] | None = None
    map_channels: list[str] = Field(default_factory=list)
    feature_names: list[str] = Field(default_factory=list)
    embedding_slot: int | None = None
    social_context: Mapping[str, Any] = Field(default_factory=dict)
    local_summary: Mapping[str, float] = Field(default_factory=dict)
    personality: Mapping[str, Any] = Field(default_factory=dict)
    landmark_slices: Mapping[str, tuple[int, int]] = Field(default_factory=dict)
    compact: Mapping[str, Any] = Field(default_factory=dict)
