...
class QueueConflictTracker:
    ...
    def reset(self) -> None:
        """Clear any queued conflict telemetry snapshots."""
    ...
    def remove_agent(self, agent_id: str) -> None:
        """Remove pending conflict events referencing the provided agent."""

        if self._rivalry_events:
            filtered = [
                event
                for event in self._rivalry_events
                if event.get("agent_a") != agent_id and event.get("agent_b") != agent_id
            ]
            self._rivalry_events = deque(filtered, maxlen=self._rivalry_events.maxlen)

        if self._chat_events:
            self._chat_events = [
                event
                for event in self._chat_events
                if event.get("speaker") != agent_id and event.get("listener") != agent_id
            ]

        if self._avoidance_events:
            self._avoidance_events = [
                event
                for event in self._avoidance_events
                if event.get("agent") != agent_id and event.get("target") != agent_id
            ]
